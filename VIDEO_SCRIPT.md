...
> **⚠️ IMPORTANT:** This video covers the OpenHands SDK's implementation of dynamic workflows,
> which is inspired by (but not identical to) Claude Code's Dynamic Workflows.
> We don't have Anthropic's source code — this is our interpretation using open source tools.
...
**[Say: "Quick note — this is the OpenHands SDK's implementation of a pattern Anthropic described. We don't know exactly how they do it, but this is our version using open source tools."]**
...
# ============================================================
# THE SKILL: This is what guides the agent
# ============================================================
ORCHESTRATOR_SKILL = Skill(
    name="orchestrator",
    content="""
    ...
    4. Structure the final report with Executive Summary, Key Findings, Sources
    """
)

# Register sub-agent types (templates for different roles)
...
# Create parent agent with workflow tool AND the skill
parent_agent = Agent(
    llm=LLM(model="gpt-4o-mini"),
    tools=[Tool(name=WorkflowToolSet.name)],  # <-- THE MAGIC
    agent_context=AgentContext(
        skills=[ORCHESTRATOR_SKILL]  # <-- Guides the agent on HOW to structure workflow
    )
)
...
# ============================================================
# WHAT THE AGENT FIGURED OUT (based on the skill):
# ============================================================
# - "The question is about market size. I should research:
#     1. Market overview and major players
#     2. Technical capabilities
#     3. Expert opinions
#     4. Statistics and data
#     5. Concerns/debates"
# - Then fan out to web_searcher agents
# - Cross-check with fact_checker agents
# - Synthesize with synthesizer agent
```
...
>
> "But here's the key insight — **the skill tells the agent HOW to structure the workflow**, including what research angles to use."
>
...
> "The agent looked at the question — 'What is the AI coding assistant market size in 2024?' — and decided:"
>
> "I need to research this from multiple angles. Let me break it down:"
>
> "**Angle 1:** Market overview and major players (who are the key companies?)"
> "**Angle 2:** Technical capabilities (what can these tools actually do?)"
> "**Angle 3:** Expert opinions (what do industry leaders say?)"
> "**Angle 4:** Statistics and data (what are the numbers?)"
> "**Angle 5:** Concerns and debates (what are the counterarguments?)"
>
> "The skill gave it the framework. The agent filled in the specifics based on the question."
